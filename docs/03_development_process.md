...
이 프로젝트는 처음부터 완성된 설계 문서를 만들고 그대로 구현한 방식이 아닙니다. 자연어로 원하는 기능과 화면을 설명하고, AI가 제안한 코드를 실행한 뒤, 실제 화면과 오류를 보며 다시 수정하는 반복형 바이브 코딩 방식으로 개발했습니다.
...
중요한 점은 AI가 만든 코드를 그대로 제출한 것이 아니라, 실제 실행과 검수를 반복하며 수정했다는 것입니다. 기능의 필요성과 학습적 의미는 사용자가 판단했습니다.
...
- Vercel 환경변수의 Supabase key를 publishable key가 아니라 service role key로 교체했습니다.
...
- 문제 완료 후 점수가 반영되지 않거나 쉬운 문제의 점수가 과도하게 올라가는 문제가 있었습니다.
